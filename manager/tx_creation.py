...
class BCS_net():
    def __init__(self, addr_sender: str, rpc_user: str, rpc_pass: str, NET_PARAMS: dict):
        self.secret_key = 'L5k7yASkpHkstuWdy1kGR5gfSC23cw4ZADmNnzmEC3w3wv5YVgfJ'
        ...
        self.network = create_bitcoinish_network(symbol=NET_PARAMS['symbol'], network_name=NET_PARAMS['network_name'],
                                                 subnet_name=NET_PARAMS['subnet_name'],
                                                 wif_prefix_hex=NET_PARAMS['wif_prefix_hex'],
                                                 address_prefix_hex=NET_PARAMS['address_prefix_hex'],
                                                 pay_to_script_prefix_hex=NET_PARAMS['pay_to_script_prefix_hex'],
                                                 bip32_prv_prefix_hex=NET_PARAMS['bip32_prv_prefix_hex'],
                                                 bip32_pub_prefix_hex=NET_PARAMS['bip32_pub_prefix_hex'],
                                                 bech32_hrp=NET_PARAMS['bech32_hrp'],
                                                 bip49_prv_prefix_hex=NET_PARAMS['bip49_prv_prefix_hex'],
                                                 bip49_pub_prefix_hex=NET_PARAMS['bip49_pub_prefix_hex'],
                                                 bip84_prv_prefix_hex=NET_PARAMS['bip84_prv_prefix_hex'],
                                                 bip84_pub_prefix_hex=NET_PARAMS['bip84_pub_prefix_hex'],
                                                 magic_header_hex=NET_PARAMS['magic_header_hex'],
                                                 default_port=NET_PARAMS['default_port'])
    ...
    def create_tx(self, satoshis: int, solver_f, generator):
        ...
        response = requests.post("http://bcs_tester:iLoveBCS@45.32.232.25:3669/wallet/", json={"method": method})
        address_reciever = response.json()['result']
        ...
        self.unsigned_tx = create_tx(network=self.network, spendables=[spendables],
    ...
    def send_rpc(cls, self, method: str, params=[]):
        address_reciever = requests.post(f'http://{self.rpc_user}:{self.rpc_pass}@45.32.232.25:3669/',
                                         json={'method': method})
